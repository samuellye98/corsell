...
class CameraRollPicker extends Component {
  ...
  appendImages(data) {
    ...
    if (assets.length > 0) {
      newState.lastCursor = data.endCursor;
  ...
  async doFetch() {
    ...
    if (Platform.OS === 'android') {
      ...
      if (status !== 'granted') {
        console.log('External storage permissions not granted.');
        return;
      }
    ...
    MediaLibrary.getAssetsAsync().then(
  ...
  renderImage(item) {
    ...
    console.log(item, 'renderImage item');
  ...
  render() {
    // console.log(this.state.selected, 'CAMERAROLL SELECTED')
    const {
      initialNumToRender,
      imageMargin,
      backgroundColor,
